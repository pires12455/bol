...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Pires\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=556996103915:+55 699610-3915\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD Pires BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Pires? Este é um recurso especial para o Pires ❌',
					ownerB: '[❗] Pires? Este é um recurso especial para o Pires ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de Pires. Registre-se para fazer amizade com o bot Pires por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Pires|17\n\n──「 PIRES BOT 」──`,
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("menu")){
			client.updatePresence(from, Presence.composing)
			reply("vc esta usando PIRES bot aproveite")
	}
...
if (messagesC.includes("bot")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/gorf.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
...
if (messagesC.includes("menu")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/men.opus');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
...
case 'menu':
                    me = client.user
                     ...
                     client.sendMessage(from, help(prefix), text, {quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "*Pires-BOT*", 'jpegThumbnail': fs.readFileSync('./assets/botlogo.webp')}}}})
break
					break
					case 'lista':
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS PIRES BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @Pires domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Pires ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do Pires:*\n\n nenhum no momento'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
